...
export function chatList(): string {
  return `
...
export function renderChatRows(): void {
  const root = document.querySelector<HTMLElement>('#chatRows');
  if (!root) return;
...
function chatRow(chat: typeof state.chats[number]): string {
...
export function renderPeople(users: typeof state.users): void {
  const root = document.querySelector<HTMLElement>('#users');
  if (!root) return;
...
export function profilePreview(): string {
