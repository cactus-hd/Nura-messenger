...
function replyBar(label, author, body, action, icon) {
    return `
    <div class="reply-bar">
...
function messageMarkup(message) {
    ...
    if (message.body)
        body += `<div>${esc(message.body).replace(/\n/g, '<br>')}</div>`;
    return `
...
export function renderContextMenu(message, x, y) {
...
function actionButton(action, id, icon, label, extra = '') {
